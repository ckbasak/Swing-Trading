...
def ensure_all_bots_running():
    bot_configs = [
        ("Strategy #1 Bot", os.path.join(PROJECT_ROOT, "strategy1"), "bot.py", "bot1.pid"),
        ("Strategy #2 Bot", os.path.join(PROJECT_ROOT, "strategy2"), "bot.py", "bot2.pid"),
        ("Strategy #3 Bot", os.path.join(PROJECT_ROOT, "strategy3"), "bot.py", "bot3.pid"),
        ("ETF Strategy #1 Bot", os.path.join(PROJECT_ROOT, "strategy_etf"), "bot.py", "bot_etf.pid"),
    ]
    
    status_dict = {}
    for name, s_dir, script, pid_f in bot_configs:
        pid_path = os.path.join(s_dir, pid_f)
        if not is_bot_alive(pid_path):
            try:
                bot_script = os.path.join(s_dir, script)
                proc = subprocess.Popen([sys.executable, "-u", bot_script], cwd=s_dir, env=os.environ.copy())
                with open(pid_path, "w") as f:
                    f.write(str(proc.pid))
                print(f"Spawned {name} daemon (PID: {proc.pid})")
...
if "Master Multi-System" in active_system:
    ...
    s1_path = os.path.join(PROJECT_ROOT, "strategy1")
    if s1_path not in sys.path:
        sys.path.insert(0, s1_path)
    os.chdir(s1_path)
    import strategy1.app as s1_app

elif "Strategy #2" in active_system:
    s2_path = os.path.join(PROJECT_ROOT, "strategy2")
    if s2_path not in sys.path:
        sys.path.insert(0, s2_path)
    os.chdir(s2_path)
    import strategy2.app as s2_app

elif "Strategy #3" in active_system:
    s3_path = os.path.join(PROJECT_ROOT, "strategy3")
    if s3_path not in sys.path:
        sys.path.insert(0, s3_path)
    os.chdir(s3_path)
    import strategy3.app as s3_app

elif "ETF Strategy" in active_system:
    setf_path = os.path.join(PROJECT_ROOT, "strategy_etf")
    if setf_path not in sys.path:
        sys.path.insert(0, setf_path)
    os.chdir(setf_path)
    import strategy_etf.app as setf_app
