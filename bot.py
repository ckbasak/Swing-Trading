...
async def news_action(chat_id: int, context: ContextTypes.DEFAULT_TYPE, query_arg: str = None):
    """
    Executes Comprehensive Global & Indian Market News Analysis & Guardrails (Strategy #2):
            ...
            if portfolio_manager.is_schedule_due(item, now):
                ...
                try:
                    if mode in ("SENTIMENT", "NEWS"):
                        ...
                        header = (
                            f"📰 *Dynamic Scheduled AI News Sentiment Briefing (Google Sheets Trigger) — Strategy #2*\n"
