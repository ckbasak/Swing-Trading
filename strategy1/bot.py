...
async def strategy_action(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        specs_text = """• **Target Universe**: NSE Nifty 50 Index Constituents (Blue Chips)
• **Entry Trigger**: Today Close > 20 SMA & Yesterday Close <= 20 SMA
• **Volume Filter**: Today Volume > 2.0x 20-day Volume SMA
• **RSI Filter**: 14-period RSI between 50 and 70 (inclusive)
• **Risk Management**: 1.0% Capital Risk per Trade | 1:2 Fixed Risk-to-Reward
• **Stop-Loss Buffer**: Clamped between 3.0% and 15.0%
• **Holding Defense**: 20 EMA Trailing Stop Loss"""
        
        msg = f"⚡ **Strategy #1: Classic 20 SMA Breakout**\n\n"
...
async def rates_action(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        lines = [
            f"🏛️ **Active Regulatory Charges & Tax Schedule (Strategy #1)**",
            "────────────────────────────",
...
async def schedules_action(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"⏳ *Checking Google Sheets scan schedules as of {now.strftime('%H:%M:%S IST')}...*",
        parse_mode="Markdown"
    )
    ...
    try:
        ...
        if open_pos:
            ...
            for idx in range(analyzed_count):
                ...
                report = sentiment_analyzer.format_detailed_sentiment_report(detailed)
                holding_header = f"💼 *Active Holding #{idx+1} of {len(tickers)}*\n"
                await context.bot.send_message(
                    chat_id=chat_id,
...
async def regulatory_sentinel_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    """
    logger.info(f"Starting scheduled Regulatory & Tax Sentinel news check (Strategy #1)...")
    loop = asyncio.get_event_loop()
    try:
        client = await loop.run_in_executor(None, portfolio_manager.get_gspread_client)
        sh = await loop.run_in_executor(None, lambda: portfolio_manager.get_or_create_portfolio_sheet(client))
