...
async def scan_action(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await send_long_message(context, chat_id, f"❌ Error running scan: {e}", reply_markup=get_main_keyboard())
...
async def strategy_action(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        specs_text = """• **Target Universe**: Curated High-Momentum Growth Pool (Nifty Midcap 100 / 500)
• **Entry Trigger**: High-Volume Spikes (> 2.25x 20-day Vol SMA)
• **Stop-Loss Buffer**: Initial Stop Loss at 2.0x ATR
• **Target 1 (50% Lock)**: +2.0x ATR (~+6-7% gain)
• **Break-Even Guard**: Stop Loss automatically moves to Entry Price after T1!
• **Target 2 (Runner)**: 20 EMA Trailing up to +4.5x ATR
• **Risk Allocation**: 6.0% Risk per Trade | Max 10 Open Positions"""
        
        msg = f"⚡ **Strategy #3: Hybrid Optimal Momentum**\n\n"
