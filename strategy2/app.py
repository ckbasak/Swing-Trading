...
if st.sidebar.button("🔄 Sync & Refresh Portfolio"):
    ...
    with st.spinner("Syncing portfolio targets and trailing stops..."):
        try:
            ...
            time.sleep(2)  # Pause to let Google Sheets API quota window settle
        except Exception as e:
            st.sidebar.error(f"Error syncing portfolio: {e}")

if st.sidebar.button("🧹 Reset Account for Fresh Start"):
    st.cache_data.clear()
...
def load_data():
    ...
    for attempt, delay in enumerate(delays):
        ...
        try:
            client = portfolio_manager.get_gspread_client()
            if not client:
                st.warning("⚠️ **Google Sheets Notice**: Service Account credentials invalid or unconfigured (`invalid_grant`). Using local/Dhan fallback portfolio.")
                return fallback_account, [], []

            sh = portfolio_manager.get_or_create_portfolio_sheet(client)
            if not sh:
                st.warning("⚠️ **Google Sheets Notice**: Unable to access Google Sheet database (`invalid_grant`). Using local/Dhan fallback portfolio.")
                return fallback_account, [], []
            ...
            return account or fallback_account, holdings or [], schedules or []
        except Exception as e:
            err_str = str(e)
            if ("429" in err_str or "Quota exceeded" in err_str or "RESOURCE_EXHAUSTED" in err_str or "Quota" in err_str) and attempt < len(delays) - 1:
                continue
            
            st.warning(f"⚠️ **Google Sheets Notice**: Operating in fallback mode ({e}).")
            return fallback_account, [], []
...
if account is not None and holdings is not None:
    ...
    with tab_open:
        ...
        if not open_df.empty:
            ...
            display_columns = [
                "Ticker", "Company Name", "Sector", "Entry Date", "Entry Price", "Quantity", "Entry Value",
