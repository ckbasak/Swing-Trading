...
* `🌐 /news` — Generates in-depth **Market Sentiment & Macro Guardrails Reports** powered by **Gemini 3.6-flash**:
  * `/news`: Automatically analyzes market sentiment and macro guardrails for all active open Strategy #2 holdings (or Nifty 50 benchmark if no open holdings).
...
| **Telegram Bot** | `TELEGRAM_BOT_TOKEN_2` | `<YOUR_TELEGRAM_BOT_TOKEN>` | Render Environment |
